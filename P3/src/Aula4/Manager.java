...
public class Manager {
	...
	public static void add(String cc, Pessoa p){
		if(!pessoas.containsKey(cc)){
			pessoas.put(cc, p);
		}else{
			System.out.println("Nœmero de cc duplicado, imposs’vel adicionar!");
		}
	}
	
	public static void delete(String cc){
		if(pessoas.isEmpty()){
			System.out.println("Lista vazia, imposs’vel eliminar! ");
		}else{
			if(pessoas.containsKey(cc)){
				pessoas.remove(cc);
			}else{
				System.out.println("Nœmero de cc n‹o existe, imposs’vel eliminar!");
			}
		}
	}
