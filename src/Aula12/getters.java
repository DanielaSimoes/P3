...
public class getters {
	...
	public void doMethods(int j, Object obj) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
		Scanner sc = new Scanner(System.in);
		...
		Method m = list.get(j);
		System.out.println("Metodo selecionado " + m);
		
		Class<?>[] paramTypes = m.getParameterTypes();
		Object[] paramsObj = new Object[paramTypes.length];
		
		for (int k = 0; k < paramTypes.length; k++) {
			System.out.println(paramTypes[k].getSimpleName()+": ");
			
			switch(paramTypes[k].getSimpleName()){
			case "double":
				paramsObj[k] = sc.nextDouble();
				break;
			case "int":
				paramsObj[k] = sc.nextInt();
				break;
			case "char":
				paramsObj[k] = sc.next().charAt(0);
				break;
			case "String":
				paramsObj[k] = sc.next();
				break;
		...
		System.out.println(m.invoke(obj, paramsObj));
	}	
}
